...
public class StockMarketSimulator {
    ...
    public static final String HISTORYFILEPATH = "C:\\Users\\harsh\\Desktop\\Latest\\Stock-Market-Simulator-master\\dist\\history";
    ...
    public static accountProfile accountCreation() {
        ...
        do {
            // Variables are synchronized to break from the do-while loop 
    ...
    // Modified decreasePrice method to ensure prices don't go negative
    public static void decreasePrice(stockProfile profile, double priceChange) {
        double sellChange = profile.getSellPrice()
                - (priceChange + ((profile.getSellPrice() * profile.getMargin()) / 2));
        double buyChange = profile.getBuyPrice()
                - (priceChange + ((profile.getBuyPrice() * profile.getMargin()) / 2));

        // Ensure the price doesn't go negative
        if (sellChange < 0) sellChange = 0.01; // Set a small minimum price
        if (buyChange < 0) buyChange = 0.01; // Set a small minimum price
    ...
    public static stockProfile[][] createAllStocks() {
        ...
        double[][] currencyPrice = {{1.2183, 1.2185}, {1.3767, 1.3768}, {0.88491, 0.88511}, {147.279, 147.320}};
